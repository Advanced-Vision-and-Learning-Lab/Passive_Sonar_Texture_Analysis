...
def main(Params, optimize=False):
    ...
    for split in range(0, 2):
        ...
        if Dataset == 'DeepShip':
            ...
            ## Data loader for VTUAD meta files
            # data_module = DeepShipDataModule(
        ...
        # Filter the state_dict to only include keys with '1' and remove '1.' prefix
        # teacher_state_dict = {k[2:] if k.startswith("1.") else k: v for k, v in model_ft.model.state_dict().items() if k.startswith("1.")}        
        # #Save the modified state dictionary
        # torch.save(teacher_state_dict, os.path.join(filename, 'teacher_model_weights.pth'))
        # pdb.set_trace()
...
def parse_args():
    ...
    parser.add_argument('--folder', type=str, default='Saved_Models/e/', help='Location to save models')
    ...
    parser.add_argument('--train_batch_size', type=int, default=8, help='input batch size for training (default: 128)')
