...
# Combine statistical and frequency features
X_combined = np.hstack((np.vstack(df['Statistical Features'].values), np.vstack(df['Frequency Features'].values))).astype(np.float32)
y = df['Label'].values
...
X_train_combined, X_val_combined, X_test_combined, y_train, y_val, y_test = custom_train_val_test_split(X_combined, y_encoded, train_size, val_size, test_size)
...
# Print dataset info
print("Training set size:", X_train_combined.shape[0])
print("Validation set size:", X_val_combined.shape[0])
print("Test set size:", X_test_combined.shape[0])
...
train_data_combined = TensorDataset(torch.from_numpy(X_train_combined), torch.from_numpy(y_train))
val_data_combined = TensorDataset(torch.from_numpy(X_val_combined), torch.from_numpy(y_val))
test_data_combined = TensorDataset(torch.from_numpy(X_test_combined), torch.from_numpy(y_test))

batch_size = 32
train_loader = DataLoader(train_data_combined, batch_size=batch_size, shuffle=True)
val_loader = DataLoader(val_data_combined, batch_size=batch_size)
test_loader = DataLoader(test_data_combined, batch_size=batch_size)

from sklearn.decomposition import PCA
# Performing PCA to reduce GLCM features to 2 dimensions for visualization
pca = PCA(n_components=2)
X_train_pca_combined = pca.fit_transform(X_train_combined)

# Plotting the PCA results
plt.figure(figsize=(8, 6))
for i, ship_type in enumerate(label_encoder.classes_):
    plt.scatter(X_train_pca_combined[y_train == i, 0], X_train_pca_combined[y_train == i, 1], label=ship_type)
plt.xlabel('Principal Component 1')
plt.ylabel('Principal Component 2')
plt.title('PCA of Combined Features (training data)')
plt.legend()
plt.savefig('../figures_modulated/PCA_combined_Features_V4.png', dpi=300, bbox_inches='tight')
...
def run_experiment(run_id, device='cuda'):
    ...
    plt.title(f'Loss Plot Combined - Run {run_id}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'../figures_modulated/loss_plot_combined_run_{run_id}.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.title(f'Confusion Matrix Combined - Test Data Run {run_id}')
    plt.xlabel('Predicted Label')
    plt.ylabel('True Label')
    plt.savefig(f'../figures_modulated/confusion_matrix_combined_run_{run_id}.png', dpi=300, bbox_inches='tight')
    plt.close()
...
print("Model A (Combined) Training Done!")
