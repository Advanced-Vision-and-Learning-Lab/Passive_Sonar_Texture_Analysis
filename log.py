...
def extract_metrics(log_dir, iterations_per_epoch):
    ...
    for event_file in event_files:
        ...
        for tag in tags['scalars']:
            events = event_acc.Scalars(tag)
            for e in events:
                # Dynamically compute epochs from iteration steps
                epoch = e.step // iterations_per_epoch
...
def aggregate_metrics(runs_dirs):
    ...
    for run_dir in runs_dirs:
        # print(f"Checking directory: {run_dir}")
        metrics = extract_metrics(run_dir, 580)
        if metrics:
            for key, values in metrics.items():
                all_metrics[key].append(values)
    return all_metrics
...
def compute_stats(all_metrics):
    stats = {}
    for key, values_list in all_metrics.items():
        all_values = [value for values in values_list for _, value in values if not np.isnan(value)]  # Exclude nan
        if all_values:
            mean = np.mean(all_values)
            std = np.std(all_values)
            stats[key] = {'mean': mean, 'std': std}
...
def plot_metrics_for_each_run(all_metrics, metric_name_pairs, run_index):
    for train_metric_name, val_metric_name, title in metric_name_pairs:
        ...
        print(f"Checking {train_metric_name} for run {run_index}")
        if train_metric_name in all_metrics:
            print(f"Available runs for {train_metric_name}: {len(all_metrics[train_metric_name])}")
        if val_metric_name in all_metrics:
            print(f"Available runs for {val_metric_name}: {len(all_metrics[val_metric_name])}")
        ...
        train_epochs = [epoch for epoch, value in train_values if not np.isnan(value)]
        train_values = [value for _, value in train_values if not np.isnan(value)]
        val_epochs = [epoch for epoch, value in val_values if not np.isnan(value)]
        val_values = [value for _, value in val_values if not np.isnan(value)]

        if not train_epochs or not val_epochs:
            print(f"No valid data for {train_metric_name} or {val_metric_name} in run {run_index + 1}")
            continue

        # Plot single data points if only one epoch is available
        if len(train_epochs) == 1:
            plt.scatter(train_epochs, train_values, label='Train', color='blue')
        else:
            plt.plot(train_epochs, train_values, label='Train', color='blue')

        if len(val_epochs) == 1:
            plt.scatter(val_epochs, val_values, label='Val', color='orange')
        else:
            plt.plot(val_epochs, val_values, label='Val', color='orange')

        plt.xlabel('Epochs')
        plt.ylabel(title.replace('_', ' ').title())
        plt.legend()
        plt.title(f'{title.replace("_", " ").title()} for Run 2')
        plt.show()
...
# Example usage:
runs_dirs = [
    '/home/grads/j/jarin.ritu/Documents/Research/SSTKAD_Lightning/Saved_Models/SSATKD_4k_DP/AdamW/distillation/Fine_Tuning/DeepShip/TDNN_MobileNetV1/Run_1/tb_logs/model_logs/version_0'
]
all_metrics = aggregate_metrics(runs_dirs)
stats = compute_stats(all_metrics)
