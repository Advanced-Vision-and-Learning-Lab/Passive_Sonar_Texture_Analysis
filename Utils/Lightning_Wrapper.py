...
class Lightning_Wrapper(L.LightningModule):
    def __init__(self, model, num_classes, max_iter,log_dir, optimizer=optim.AdamW, lr=0.00001,
    ...
    def configure_optimizers(self):
        ...
        # Learning rate decay every 2 steps with a decay rate of 0.9
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, 
            lr_lambda=lambda step: 0.9 ** (step // 2))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # pdb.set_trace()
        # flops = FlopCountAnalysis(self.model, signals)
        # flops.total()
        # pdb.set_trace()
