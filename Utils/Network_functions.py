...
def initialize_model(mode,student_model,teacher_model, in_channels, out_channels, use_pretrained=False, num_classes = 1, feature_extract=False,
    ...
    # feature_layer = Feature_Extraction_Layer(input_feature=input_feature,sample_rate=sample_rate, window_length=4096,
    ...
    if mode == 'student' or mode == 'distillation':
        ...
        if student_model == "TDNN":
            ...
            checkpoint_path = './VUATD_weights/student_model_weights2k.pth'
            checkpoint = torch.load(checkpoint_path)
            # Remove the '0.' or '1.' prefix from keys
            new_state_dict = {}
            for key, value in checkpoint.items():
                new_key = key.split('.', 1)[-1]  # Removes the first numeric prefix if it exists
    ...
    if mode == 'teacher' or mode == 'distillation':
        ...
        if teacher_model == "CNN_14":
            ...
            if use_pretrained: #Pretrained on AudioSet
                teacher_model_ft = MobileNetV1(sample_rate=32000, window_size=1024, hop_size=320, mel_bins=101, fmin=50, 
                    fmax=14000, classes_num=5)
            
                teacher_state_dict = torch.load('./VUATD_weights/teacher_model_weights4k.pth')
                # pdb.set_trace()
            ...
            if use_pretrained:
                ...
                teacher_model_ft = MobileNetV1(sample_rate=32000, window_size=1024, hop_size=160, mel_bins=64, fmin=0, 
                    fmax=None, classes_num=num_classes)
        elif teacher_model == "SimpleCNN":
            teacher_model_ft = SimpleCNN()
            set_parameter_requires_grad(teacher_model_ft, feature_extract)
            num_ftrs_t = teacher_model_ft.fc1.in_features
            teacher_model_ft.fc1 = nn.Linear(num_ftrs_t, 128)
            # input_size = 224
        ...
        #Create feature extaction layer for project networks 
        # feature_layer.bn = nn.BatchNorm2d(teacher_model_ft.bn0.num_features)  
        # num_ftrs_t = teacher_model_ft.fc1.in_features
        # teacher_model_ft.fc1 = nn.Linear(num_ftrs_t,num_classes)
